...
imwrite(edgeSmooth, 'ps1-6-a-output0-edges.png');
...
for i = 1:length(x)
    for j = 17:45
        for t = theta
            x0 = ceil(x(i) + j*cos(t));
            y0 = ceil(y(i) + j*sin(t));
            array(x0+44,y0+1,j-16) = array(x0+44,y0+1,j-16) + 1;
        end
    end
end
...
index1 = find(array>max(array(:))-60);
...
index2 = find(toShow>max(toShow(:))-40);
